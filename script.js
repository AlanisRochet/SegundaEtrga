class Recet{
    constructor(nombre, ingredientes, receta){
        this.nombre=nombre;
...
recetas.forEach(el =>{
    ...
    nodo.innerHTML =`
    <h3>${el.nombre}</h3>
    <h4>${el.ingredientes}</h4>
    <p>${el.receta}</p>
    `
    document.getElementById("receta").appendChild(nodo);
}
)
...
const filtrar = ()=> {
  ...
  for(let receta of recetas){
    ...
    if(ingrediente.indexOf(texto)!== -1){
        resultado.innerHTML += `
        <li>${receta.nombre} -Ingredientes${receta.ingredientes}- Receta: ${receta.receta}</li>
        `
    }
  }
